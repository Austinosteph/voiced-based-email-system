...
export const VoiceCommandProvider = ({ children }: { children: ReactNode }) => {
  ...
  const { text, isListening, startListening, stopListening, resetText } = useVoiceRecognition();
  ...
  useEffect(() => {
    ...
    const processCommand = async () => {
      ...
      // Navigation commands
      if (lowerText.includes('go to inbox') || lowerText.includes('show inbox')) {
        speak("Opening inbox");
        navigate('/dashboard');
      } 
      else if (lowerText.includes('go to compose') || lowerText.includes('new email') || lowerText.includes('compose email')) {
      ...
      else if (lowerText.includes('log out') || lowerText.includes('sign out')) {
        speak("Logging you out");
        // The actual logout will be handled by the auth context
      }
      // Email actions
      else if (lowerText.includes('read email') || lowerText.includes('read message')) {
      ...
      else if (lowerText.includes('delete email') || lowerText.includes('delete message')) {
        const emailId = extractEmailId(lowerText);
        if (emailId) {
      ...
      else if (lowerText.includes('refresh') || lowerText.includes('update inbox')) {
